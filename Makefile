...
create-migrations: ## Создать миграции
	$(API_SH) "su - node -c 'cd $(API_PATH) && npm run migrate:dev:create'"

migrate: ## Выполнить миграции
	$(API_SH) "su - node -c 'cd $(API_PATH) && npm run migrate:deploy'"

create-prisma-client: ## Создать клиент призмы
	$(API_SH) "su - node -c 'cd $(API_PATH) && npm run prisma:generate'"

create-user: ## Создать пользователя (make create-user email="email" password="password")
	$(API_SH) "su - node -c 'cd $(API_PATH) && npm run build && node ./dist/commander.js create-user -e $(email) -p $(password)'"
